...
const FormModal = ({ isOpen, close, editItem }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // new ile oluşturdğumuz örnek bizim bazı fonksiyların kullanmamızı sağlar
    const formData = new FormData(e.target);

    // 2) inputlardaki veirlerden bir obje oluşturk
    const task = Object.fromEntries(formData.entries());

    if (editItem) {
      // 3) güncellenicek eleman varsa editTask aksiyonunu çalıştır
